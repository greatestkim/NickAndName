...
import { CheckBox, CustomModal, CustomText, FlexBox, GridView, InputBox } from "../components";
...
const StyledBtn = styled(FlexBox).attrs({
    ...
    margin-right: 5;
    margin-top: 5;
    min-width: ${Dimensions.get("window").width * 0.2};
`;
...
export default function NewNameScreen({ navigation }) {
    ...
    const [birthDayYearValue, setBirthDayYearValue] = useState(moment().format("YYYY"));
    const [birthDayMonthValue, setBirthDayMonthValue] = useState(moment().format("M"));
    ...
    const handleSave = () => {
        ...
        console.log("##3. birthday*", birthDayYearValue, birthDayMonthValue, birthDayValue);
        ...
        if ((needIdCard !== false && needIdCard !== true) || cnt === 0 || birthdayError !== "") {
    ...
    useEffect(() => {
        ...
        const isIntegerFunc = (val) => {
            let result = Number.isInteger(Number(val)) && String(Math.floor(Number(val))) === val;
            ...
            <KeyboardAvoidingView contentContainerStyle={{ flex: 1 }} enabled={true}>
                ...
                >
                    <CenterContainer>
                        ...
                        <InputBox title="소속" changeCallback={setClubText} textValue={clubText} />
                        ...
                        >
                            ...
                            <FlexBox direction="column" align="flex-start" style={{ flex: 7 }}>
